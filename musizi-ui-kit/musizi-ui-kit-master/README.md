...
```bash
pnpm add @northgreenug/musizi-ui-kit
```

> **Note:** This package is published to GitHub Packages. You may need to configure your package manager to use the GitHub registry for the `@northgreenug` scope (via `.npmrc`).
...
- Node.js 18+
- pnpm 10+ (pinned via `packageManager` field; install with `corepack enable`)
...
pnpm run test:e2e:dev   # Run Cypress e2e tests
...
```bash
pnpm run yalc:publish   # Build and publish to local yalc store
pnpm run yalc:push      # Build and push updates to linked projects
pnpm run yalc:watch     # Watch for changes and auto-push
```
